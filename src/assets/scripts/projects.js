...
async function planningsGallery() {
  ...
  let archiveListHtml = archivedFlats.map((flat) => (
    archiveCard(flat)
  )).join('');
  ...
  let paginationData = [];
  let currentPage$ = new BehaviorSubject(
...
async function getFlats() {
  ...
  const url = isDev
    ? './static/flats.json'
    : '/wp-json/wp/v2/posts?categories=4&_embed=1&per_page=100&order=desc';
        ...
        <a class="project-card" href="${link}">
            <div class="project-card__img">
                ...
                <div class="project-card__labels">
                    ${labels
                      .map((label, index) => {
                        return `<div class="project-card__label ${
                          index == 0 ? 'project-card__label--alert' : ''
                        }">${label.row}</div>`;
                      })
                      .join('')}
                </div>
            ...
            <div class="project-card__text">
                ${
                  adress
                    ? `<div class="project-card__adress color-body-description">
                    ${adress}
                </div>`
                    : ''
                }
