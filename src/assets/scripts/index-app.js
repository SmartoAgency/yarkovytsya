...
// Код, щоб для кожного проєкту відкривався відповідний блок із контактами
...
function slugify(text) {
  ...
  // 5. Заміна багатьох тире на одне
  result = result.replace(/-+/g, '-');

  // 6. Обрізка тире на початку/кінці
...
function contactScreenProjectsHandler() {
  ...
  let initialLinkToActive = links[0];
  ...
  if (initialLinkToActive) {
    initialLinkToActive.click();
  }
}
...
function fullPageDecorParalax() {
  ...
  const decoreImageToScaleValue =
    (pathToTransformWidth / decoreImg.getBoundingClientRect().width) * 1.17543859649122806;
...
function favouritesHandler() {
  ...
  function updateFavouritesCounterButtons() {
    ...
    window.dispatchEvent(
      new CustomEvent('favourites:updated', {
  ...
  function updateFavouritesSingleButtons() {
    document.querySelectorAll(SELECTORS['favouriteSingleButton']).forEach(button => {
      const flatId = button.getAttribute(
        SELECTORS['favouriteSingleButton'].replace(/(\[|\])/g, ''),
      );
...
function incredibleBlockParalax() {
  ...
  const tl = gsap
    ...
    .fromTo(
      svg.querySelectorAll('.ярковиця__Union path'),
      {
        yPercent: 100,
      },
      {
...
function aboutScreenSwipeImageHandler() {
  ...
  subscribeScrollValue(value => {
    ...
    const swipeXoffset = gsap.utils.mapRange(
      ...
      0,
      svgWidth - slideSvgButtonRadius * 2,
      value,
    );
