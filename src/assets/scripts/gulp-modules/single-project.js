...
function advantagesSliderHandler() {
  ...
  if (isDesktop) {
    ...
    mainSmallSlider.on('slideChange', function(instance) {
      setTimeout(() => {
        const nextIndex = document
          .querySelector('[data-advantages-slider-small] .swiper-slide-active')
          .getAttribute('data-real-index');
    ...
    largeSlider.on('slideChange', instance => {
      ...
      document.querySelectorAll('[data-advantages-slider-current]').forEach(item => {
        item.textContent =
          instance.realIndex + 1 < 10 ? '0' + (instance.realIndex + 1) : instance.realIndex + 1;
      });
    });
  }
  ...
  textSlider.on('transitionStart', () => {
    document
      .querySelectorAll(
        '.advantages-slider, [data-advantages-slider-prev], [data-advantages-slider-next]',
      )
      .forEach(item => {
    ...
    gsap
      .timeline({
        onComplete: function() {
          ...
          gsap.set(
            [
              prevElement.querySelector('.advantages-slider__large-item'),
...
document.body.addEventListener('click', function(evt) {
  ...
  document.querySelectorAll('[data-front-screen-line-index]').forEach(item => {
    ...
    //set css var
    item.style.setProperty('--line-time', '2s');
  });
